...
class Prepare:
    ...
    def load_data(self, name):
        #win_home="E:/text_mining/0Data_Collection/corpus/gene-disease/muti_re/"
        win_home = "E:/text_mining/0Data_Collection/corpus/ncRNA-disease/"
        if name.lower() == 'train':
            #filename = self.config.train_filename
            filename =win_home+"train.json"
        elif name.lower() == 'test':
            #filename = self.config.test_filename
            filename = win_home + "test.json"
        elif name.lower() == 'valid':
            #filename = self.config.valid_filename
            filename = win_home + "valid.json"
        else:
...
class NYTPrepare(Prepare):
        ...
        for i, a_data in enumerate(data):
            ...
            for triple in triples:
                ...
                try:
                    ...
                    print("m1:"+nltk.word_tokenize(triple['em1Text']),"m2:"+nltk.word_tokenize(triple['em2Text']))
                label = triple['label']
                if label != 'None':
                    ...
                    try:
                        ...
                        is_save = False
                        # print(sent_id.index(words2id[m1]), sent_id.index(words2id[m2]), relations2id[label])
                        print('[%s] or [%s] is not in words2id, relation is (%s)' % (m1, m2, sent_text))
    ...
    def analyse_data(self, name):
        ...
        for sent_triples in all_triples_id:
            ...
            #     print sent_triples
        print('Normal Count %d, Multi label Count %d, Overlapping Count %d' % (normal_count, multi_label_count, over_lapping_count))
...
config = ""
prepare = NYTPrepare(config)
prepare.analyse_data("train")
